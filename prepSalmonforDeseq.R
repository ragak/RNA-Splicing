...
KO1 <- read.table("KO1_exonquant.txt", sep="\t", header=TRUE)
KO2 <- read.table("KO2_exonquant.txt", sep="\t", header=TRUE)
Rev1 <- read.table("Rev1_exonquant.txt", sep="\t", header=TRUE)
Rev2 <- read.table("Rev2_exonquant.txt", sep="\t", header=TRUE)
anno <- read.table("DEseq2-results.txt", sep="\t", header=TRUE)
anno2=data.frame(anno$gene.musg,anno[,13])
...
write.table(unique(alldataanno),"Ilf2-salmonSummary-exon.txt", sep="\t", quote=FALSE)
